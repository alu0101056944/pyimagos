...
(expected_contour,
case_title,
best_precision_first_stage) to study the failure cases on radiographies that should be
...
def generate_first_ranges_main():
  ...
  def table_start(encounter_amount: int = 1):
    return (
      '\\begin{table}[H]\n') + (
      '\\centering\n') + (
      '\\caption{Valores de rango de primera fase ') + (
        f'{encounter_amount}' + '}\n') + (
      '\\label{tab:first_ranges ' + f'{encounter_amount}' + '}\n') + (
      '\\begin{footnotesize}\n') + (
      '\\begin{tabular}{|l|l|c|}\n') + (
      '\\toprule\n') + (
      '\\textbf{Contorno esperado} & \\textbf{Caso} & \\textbf{Valor de rango fase 1} \\\\ \n') + (
      '\\hline\n')
  ...
  expected_contour_to_case_to_best_first = {}
  ...
  for expected_contour_key in expected_contour_to_cases:
    expected_contour_to_case_to_best_first[expected_contour_key] = {}
    ...
    expected_contour_to_case_to_best_first[
      expected_contour_key][case_title] = best_precision_penalization_factor
  ...
  for expected_contour_key in expected_contour_to_case_to_best_first:
    for case_title in expected_contour_to_case_to_best_first[expected_contour_key]:
      best_first_stage_precision = expected_contour_to_case_to_best_first[
        expected_contour_key][case_title]
      
      case_title = case_title.replace('_', '\\_')

      table_entry = f'{expected_contour_key} & {case_title} & {best_first_stage_precision}' + (
        f' \\\\ \n')
  ...
  with open('tab_first_ranges.txt', 'w', encoding='utf-8') as f:
    f.write(output_string)
    print('Writing tab_first_ranges.txt')
    print('Success.')

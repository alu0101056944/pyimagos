...
def generate_contour_type_precisions_main():
  ...
  def table_start(encounter_amount: int = 1):
    return (
      '\\begin{table}[H]\n') + (
      '\\centering\n') + (
      '\\caption{Precisión media por cada tipo de contorno ') + (
        f'{encounter_amount}' + '}\n') + (
      '\\label{tab:contour_type_precisions ' + f'{encounter_amount}' + '}\n') + (
      '\\begin{footnotesize}\n') + (
      '\\begin{tabular}{|l|c|c|}\n') + (
      '\\toprule\n') + (
  ...
  for contour_type_key in contour_type_to_factor_to_average_precision:
    for penalization_factor in contour_type_to_factor_to_average_precision[contour_type_key]:
      processed_contour_type_key = contour_type_key.replace('_', '\\_')
      precision = contour_type_to_factor_to_average_precision[contour_type_key][penalization_factor]
      table_entry = f'{processed_contour_type_key} & {penalization_factor} & {precision}' + (
        f' \\\\ \n')
  ...
  with open('tab_contour_type_precisions.txt', 'w', encoding='utf-8') as f:
    f.write(output_string)
    print('Writing tab_contour_type_precisions.txt')
    print('Success.')
